...
def generate_showers_data_frame(
    ...
    if len(col_ind) > 0:
        ...
        energy_t = obtain_true_shower_E(energy_t, fake_showers_e, e_pred_showers)
        vertex = obtain_vertex(dic, e_pred_showers, fake_showers_e)
        pid_t = obtain_pid_true(dic, fake_showers_e, e_pred_showers)
        pos_t = obtain_pos_true(dic, fake_showers_e, e_pred_showers)
        e_reco =  obtain_reco_showers_E(dic, fake_showers_e, e_pred_showers)
        
        e_pred =  obtain_pred_showers(e_pred_showers,energy_t, row_ind_, index_matches, fake_showers_e)
        calc_pandora_momentum = "pandora_momentum" in dic["graph"].ndata
        result = obtain_matched_es_cali(number_of_showers_total, fake_showers_e, pred_ref_pt, pred_pid, pred_pos, mask, e_corr, dic, labels, pandora, energy_t, e_pred_showers, row_ind_, index_matches, calc_pandora_momentum)
    
        if pandora:
            e_pred_cali,e_pred_cali_pfo, positions_pfo, ref_pts_pfo,  number_of_showers_total, is_track = result
        else:
            e_pred_cali,e_pred_pos, e_pred_pid, e_pred_ref_pt, calibration_factor, number_of_showers_total, is_track  = result
        ...
        if pandora:
            ...
            print(pandora, energy_t.shape,e_reco.shape,e_pred_t.shape, e_pred.shape,  pid_t.shape, calibration_factor.shape)
            d = {
                "true_showers_E": energy_t.detach().cpu(),
                "reco_showers_E": e_reco.detach().cpu(),
                "pred_showers_E": e_pred.detach().cpu(),
...
def obtain_matched_es_cali(number_of_showers_total, fake_showers_e, pred_ref_pt, pred_pid, pred_pos, mask, e_corr, dic, labels, pandora, energy_t, e_pred_showers, row_ind_, index_matches, calc_pandora_momentum):
    ...
    if pandora:
        return e_pred_cali,e_pred_cali_pfo, positions_pfo, ref_pts_pfo,  number_of_showers_total, is_track
    else: 
        return  e_pred_cali,e_pred_pos, e_pred_pid, e_pred_ref_pt, calibration_factor, number_of_showers_total, is_track


def obtain_pred_showers(e_pred_showers,energy_t, row_ind_, index_matches, fake_showers_e):
    matched_es = torch.zeros_like(energy_t) * (torch.nan)
    matched_es = matched_es.to(e_pred_showers.device)
    matched_es[row_ind_] = e_pred_showers[index_matches]
    e_pred = torch.cat((matched_es, fake_showers_e), dim=0)
    print("e_pred", matched_es.shape, fake_showers_e.shape)
    return e_pred



def obtain_reco_showers_E(dic, fake_showers_e, e_pred_showers):
    fake_showers_showers_e_truw = torch.zeros((fake_showers_e.shape[0])) * (
                torch.nan
            )
...
def obtain_pid_true(dic, e_pred_showers, fake_showers_e):
    fake_showers_showers_e_truw = torch.zeros((fake_showers_e.shape[0])) * (
                torch.nan
            )
    fake_showers_showers_e_truw = fake_showers_showers_e_truw.to(
...
def obtain_pos_true(dic, e_pred_showers, fake_showers_e):
    fakes_positions = torch.zeros((fake_showers_e.shape[0], 3)) * (torch.nan)
    fakes_positions = fakes_positions.to(e_pred_showers.device)
...
def obtain_true_shower_E(energy_t, fake_showers_e, e_pred_showers):
    fake_showers_showers_e_truw = torch.zeros((fake_showers_e.shape[0])) * (
                torch.nan
            )
    fake_showers_showers_e_truw = fake_showers_showers_e_truw.to(
    ...
    print("energy_t",energy_t.shape, fake_showers_showers_e_truw.shape)
    energy_t = torch.cat(
...
def obtain_vertex(dic, e_pred_showers, fake_showers_e):
